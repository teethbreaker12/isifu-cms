...
export function UserBadge({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={`app-panel-muted rounded-lg ${compact ? 'p-3' : 'p-4'}`}>
      <div className="flex items-start gap-3">
        <div className="accent-bg grid h-9 w-9 shrink-0 place-items-center rounded-md">
          <UserCircle size={19} />
        </div>
        <div className="min-w-0">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <span className="rounded-md border border-stone-200 bg-white px-2 py-1 text-xs font-semibold text-stone-700">{user.role}</span>
