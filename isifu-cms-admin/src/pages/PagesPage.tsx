...
export function PagesPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (editingSlug) {
      await api.updatePage(editingSlug, { slug, title, seoTitle, seoDescription, blocks, published });
    } else if (admin) {
      await api.createPage({ slug, title, seoTitle, seoDescription, blocks, published });
    }
    resetForm();
    await load();
  }
    ...
    <div className="grid gap-5">
      ...
      >
        ...
        <form className="grid gap-4" onSubmit={submit}>
          ...
          <button className="w-full rounded-md bg-stone-950 px-4 py-2 text-sm font-semibold text-white sm:w-fit">{editingSlug ? t('common.update') : t('common.save')}</button>
        </form>
        )}
      </Panel>
